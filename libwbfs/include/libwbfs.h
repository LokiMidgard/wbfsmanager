...
typedef int (__stdcall*rw_sector_callback_t)(void*fp,u32 lba,u32 count,void*iobuf);
typedef void (__stdcall*progress_callback_t)(int status,int total);
typedef void (__stdcall*close_callback_t)(void*fp);
...
/*! add a wii dvd inside the partition
  ...
  @new_name: different name for imported ISO. NULL to use default name from ISO header
 */
u32 wbfs_add_disc(wbfs_t*p,read_wiidisc_callback_t read_src_wii_disc, void *callback_data,
                  progress_callback_t spinner,partition_selector_t sel,int copy_1_1, char *new_name);


float wbfs_estimate_disc(wbfs_t*p,read_wiidisc_callback_t read_src_wii_disc, void *callback_data, partition_selector_t sel, u8* header);
